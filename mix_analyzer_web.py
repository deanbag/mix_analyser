...
# Temporary store for results (in-memory for simplicity)
latest_results = {}
...
def analyze_mix(target_file, reference_file):
    ...
    logger.info(f"Memory usage before loading files: {mem_before:.2f} MB")
    ...
    logger.info(f"Memory usage after loading files: {mem_after:.2f} MB")
...
def download_report():
    ...
    if os.path.exists(logo_path):
        logo = Image(logo_path, width=2*inch, height=2*inch*170/924)  # Maintain aspect ratio (924x170)
